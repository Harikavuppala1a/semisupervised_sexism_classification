...
def train_predict_new(phase,test_st,test_en,word_feats, sent_enc_feats, trainY, data_dict, model_type, num_cnn_filters, rnn_type, fname_part, loss_func, class_w, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val, stack_rnn_flag, cnn_kernel_set, m_ind, run_ind, save_folder_name, use_saved_model, gen_att, learn_rate, dropO1, dropO2, batch_size, num_epochs, save_model, bert_max_seq_length, st_variant):
    ...
    if use_saved_model  and os.path.isfile(fname_mod_op):
        ...
        if use_saved_model and os.path.isfile(fname_mod):
            print("loading model")
            model, att_mod , post_vec = get_model(model_type, data_dict['max_post_length'], data_dict['max_num_sent'], data_dict['max_words_sent'], word_feats, sent_enc_feats, learn_rate, dropO1, dropO2, num_cnn_filters, rnn_type, loss_func, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val, stack_rnn_flag, cnn_kernel_set, st_variant)
            model.load_weights(fname_mod)
        else:
            model, att_mod , post_vec = get_model(model_type, data_dict['max_post_length'], data_dict['max_num_sent'], data_dict['max_words_sent'], word_feats, sent_enc_feats, learn_rate, dropO1, dropO2, num_cnn_filters, rnn_type, loss_func, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val, stack_rnn_flag, cnn_kernel_set, st_variant)
        ...
        test_generator = TestGenerator(np.arange(test_st,test_en), word_feats, sent_enc_feats, data_dict, batch_size)
...
def generate_unlab_diversity_favour_intersection(mod_op_list, data_dict, bac_map, prob_trans_type,iter,save_path,conf_dict_com,post_rep_filename):
    ...
    *_,new_lab_div,data_pos_div, predicted_ex_div = generate_unlab_diversity(mod_op_list, data_dict, bac_map, prob_trans_type,iter,save_path,conf_dict_com,post_rep_filename)
    conf_dict_com['st_variant'] = split_variants[2]
    *_,new_lab_fav,data_pos_fav, predicted_ex_fav = generate_unlab_favour(mod_op_list, data_dict, bac_map, prob_trans_type,iter,save_path, conf_dict_com)
    intersect_div_fav_pos = [(num,value) for num, value in enumerate(data_pos_div) if value in data_pos_fav]


    for val in intersect_div_fav_pos:
    	num, pos = val
    	new_data_list_div_fav.append(data_dict['text'][pos])
    	new_data_list_sen_div_fav.append(data_dict['text_sen'][pos])
    	intersect_labels.append(new_lab_div[num])
    	intersect_data_pos.append(pos)
    conf_dict_com['st_variant'] = original_st_variant
    return new_data_list_div_fav , new_data_list_sen_div_fav, intersect_labels,intersect_data_pos,len(intersect_labels)
...
def generate_unlab_diversity_favour(mod_op_list, data_dict, bac_map, prob_trans_type,iter,save_path,conf_dict_com,post_rep_filename):
    ...
    return new_data_list_diverse_div_fav, new_data_list_sen_diverse_div_fav, pred_vals,position_diverse,len(new_data_list_diverse_div_fav)
...
def generate_unlab_favour(mod_op_list, data_dict, bac_map, prob_trans_type, iter,save_path,conf_dict_com):
    ...
    return new_data_list_favour, new_data_list_sen_favour, y_pred_vals,position_favour,len(y_pred_vals)
...
def generate_unlab_diversity(mod_op_list, data_dict, bac_map, prob_trans_type,iter,save_path,conf_dict_com,post_rep_filename):
    ...
    return new_data_list_diverse, new_data_list_sen_diverse, pred_vals,position_diverse,len(new_data_list_diverse)
...
def generate_unlab_IPSPC(mod_op_list, data_dict, bac_map, prob_trans_type,iter,save_path, conf_dict_com):
    ...
    return new_data_list, new_data_list_sen, pred_vals,data_pos, count_predicted
...
def sent_enc_featurize_new(sent_enc_feats_raw, model_type, data_dict, poss_sent_enc_feats_emb_dict, use_saved_sent_enc_feats, save_sent_enc_feats, data_fold_path, save_fold_path, test_mode):
...
def prep_unlab(iter, data_dict, conf_dict_com, conf_dict_list, post_vecs):
                ...
                for model_type in conf_dict["model_types"]:
                    for word_feats_raw in conf_dict["word_feats_l"]:
                        ...
                        for sent_enc_feats_raw in conf_dict["sent_enc_feats_l"]:
                            sent_enc_feats, sent_enc_feat_str, s_filename = sent_enc_featurize_new(sent_enc_feats_raw, model_type, data_dict, conf_dict_com['poss_sent_enc_feats_emb_dict'], conf_dict_com['use_saved_sent_enc_feats'], conf_dict_com['save_sent_enc_feats'], conf_dict_com["data_folder_name"], conf_dict_com["save_folder_name"], conf_dict_com["test_mode"])
                                                ...
                                                for att_dim in conf_dict["att_dims"]:
                                                    for stack_rnn_flag in conf_dict["stack_rnn_flags"]:
                                                        ...
                                                        for thresh in conf_dict["threshes"]:
                                                            ...
                                                            info_str = "iter: %s,st_variant: %s,confidence_thr:%s, retaining ratio:%s, model: %s, word_feats = %s, sent_enc_feats = %s, prob_trans_type = %s, class_imb_flag = %s, num_cnn_filters = %s, cnn_kernel_set = %s, rnn_type = %s, rnn_dim = %s, att_dim = %s, max_pool_k_val = %s, stack_rnn_flag = %s, thresh = %s, test mode = %s, avg_thrs =%f, percat_thrs = %f" % (iter,conf_dict_com['st_variant'],conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],model_type,word_feat_str,sent_enc_feat_str,prob_trans_type,class_imb_flag,num_cnn_filters,cnn_kernel_set,rnn_type,rnn_dim,att_dim,max_pool_k_val,stack_rnn_flag, thresh, conf_dict_com["test_mode"], conf_dict_com["min_num_pred_labs"],conf_dict_com["confidence_thr"])
                                                            fname_part = ("%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s" % (model_type,word_feat_str,sent_enc_feat_str,prob_trans_type,class_imb_flag,num_cnn_filters,cnn_kernel_set_str,rnn_type,rnn_dim,att_dim,max_pool_k_val,stack_rnn_flag, conf_dict_com['st_variant'],conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],iter,conf_dict_com["test_mode"]))
                                                            ...
                                                            if iter != conf_dict_com["num_iter"] - 1 :                                                                                                                                    
                                                                ...
                                                                test_en = data_dict['testunlab_en_ind']
                                                                mod_op, att_op = train_predict_new('gen_unlab',test_st,test_en,word_feats, sent_enc_feats, trainY, data_dict, model_type, num_cnn_filters, rnn_type, fname_part, loss_func, cw, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val,stack_rnn_flag,cnn_kernel_set, m_ind, best_run, conf_dict_com["save_folder_name"], conf_dict_com["use_saved_model"], conf_dict_com["gen_att"], conf_dict_com["LEARN_RATE"], conf_dict_com["dropO1"], conf_dict_com["dropO2"], conf_dict_com["BATCH_SIZE"], conf_dict_com["EPOCHS"], conf_dict_com["save_model"], conf_dict_com['bert_max_seq_len'], conf_dict_com["st_variant"])
                                                                mod_op_list.append((mod_op, att_op))
                                                                if conf_dict_com['st_variant'].startswith('basic'):
                                                                    new_data, new_data_sen, new_lab,data_pos, predicted_ex = generate_unlab_IPSPC(mod_op_list, data_dict, bac_map, prob_trans_type,iter,conf_dict_com['save_folder_name'], conf_dict_com)
                                                                elif conf_dict_com['st_variant'] == 'conf_thr':
                                                                    new_data, new_data_sen, new_lab,data_pos, predicted_ex = generate_unlab_basic(mod_op_list, data_dict, bac_map, prob_trans_type,iter,conf_dict_com['save_folder_name'], conf_dict_com)
                                                                elif conf_dict_com['st_variant'].startswith('diversity'):
                                                                    new_data, new_data_sen, new_lab,data_pos, predicted_ex = generate_unlab_diversity(mod_op_list, data_dict, bac_map, prob_trans_type,iter,conf_dict_com['save_folder_name'], conf_dict_com,post_rep_filename)
                                                                elif conf_dict_com['st_variant'].startswith('support'):
                                                                    new_data, new_data_sen, new_lab,data_pos, predicted_ex = generate_unlab_favour(mod_op_list, data_dict, bac_map, prob_trans_type,iter,conf_dict_com['save_folder_name'], conf_dict_com)
                                                                elif conf_dict_com['st_variant'].startswith('combine'):
                                                                    new_data, new_data_sen, new_lab,data_pos, predicted_ex = generate_unlab_diversity_favour(mod_op_list, data_dict, bac_map, prob_trans_type,iter,conf_dict_com['save_folder_name'],conf_dict_com,post_rep_filename)
                                                                elif conf_dict_com['st_variant'].startswith('intersection'):
                                                                    new_data, new_data_sen, new_lab,data_pos, predicted_ex =  generate_unlab_diversity_favour_intersection(mod_op_list, data_dict, bac_map, prob_trans_type,iter,conf_dict_com['save_folder_name'],conf_dict_com,post_rep_filename)  
                                                                elif conf_dict_com['st_variant'].startswith('random'):
                                                                    new_data, new_data_sen, new_lab,data_pos, predicted_ex = generate_unlab_random(mod_op_list, data_dict, bac_map, prob_trans_type,iter,conf_dict_com['save_folder_name'],conf_dict_com)
                                                                f_tsv.write("%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%s\t%s\t%s\t%s\t%s\t%s\n" % (best_run,conf_dict_com['st_variant'], conf_dict_com['max_num_pred_labs'], conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],iter,predicted_ex ,model_type,word_feat_str,sent_enc_feat_str,prob_trans_type,class_imb_flag,num_cnn_filters,cnn_kernel_set_str,thresh,rnn_dim,att_dim,max_pool_k_val,stack_rnn_flag,(metr_dict['avg_fl_ma']+metr_dict['avg_fi'])/2,(metr_dict['std_fl_ma']+metr_dict['std_fi'])/2,metr_dict['avg_fi'],metr_dict['avg_fl_ma'],(metr_dict['avg_fl_ma']+metr_dict['avg_fi']+metr_dict['avg_ji']+metr_dict['avg_fl_mi'])/4,metr_dict['avg_ji'],metr_dict['avg_fl_mi'],metr_dict['avg_em'],metr_dict['avg_ihl'],rnn_type,conf_dict_com["LEARN_RATE"],conf_dict_com["BATCH_SIZE"],conf_dict_com["dropO1"],conf_dict_com["dropO2"], conf_dict_com["test_mode"]))
                                                                ...
                                                                data_dict_filename = ("%s%s_%s_%f_%f_%d.pickle" % (conf_dict_com["save_folder_name"], 'raw_data', conf_dict_com['st_variant'],conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],iter+1))
                                                                index_filename = ("%s%s_%s_%f_%f_%d.txt" % (conf_dict_com["save_folder_name"], 'index', conf_dict_com['st_variant'],conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],iter+1))
                                                                ...
                                                                with open(data_dict_filename, 'wb') as f_data:
                                                                	pickle.dump(data_dict, f_data)
                                                                return data_pos,s_filename, len_data_train,post_rep_filename
                                                            else:
                                                            	predicted_ex = 0
                                                            	f_tsv.write("%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%s\t%s\t%s\t%s\t%s\t%s\n" % (best_run,conf_dict_com['st_variant'], conf_dict_com['min_num_pred_labs'], conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],iter,predicted_ex ,model_type,word_feat_str,sent_enc_feat_str,prob_trans_type,class_imb_flag,num_cnn_filters,cnn_kernel_set_str,thresh,rnn_dim,att_dim,max_pool_k_val,stack_rnn_flag,(metr_dict['avg_fl_ma']+metr_dict['avg_fi'])/2,(metr_dict['std_fl_ma']+metr_dict['std_fi'])/2,metr_dict['avg_fi'],metr_dict['avg_fl_ma'],(metr_dict['avg_fl_ma']+metr_dict['avg_fi']+metr_dict['avg_ji']+metr_dict['avg_fl_mi'])/4,metr_dict['avg_ji'],metr_dict['avg_fl_mi'],metr_dict['avg_em'],metr_dict['avg_ihl'],rnn_type,conf_dict_com["LEARN_RATE"],conf_dict_com["BATCH_SIZE"],conf_dict_com["dropO1"],conf_dict_com["dropO2"], conf_dict_com["test_mode"]))
                                                            	data_pos = []
                                                            	return data_pos,s_filename, len_data_train, post_rep_filename
...
for i in range(conf_dict_com["num_iter"]):
	...
	data_pos,s_filename, len_data_train,post_rep_filename = prep_unlab(i, data_dict, conf_dict_com, conf_dict_list, post_vecs)
	path_rearr_elmo = conf_dict_com['save_folder_name'] + 'word_vecs~elmo/False/'   
